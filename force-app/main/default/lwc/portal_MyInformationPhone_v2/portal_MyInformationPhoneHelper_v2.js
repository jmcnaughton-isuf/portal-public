...
export class ComponentLogic {
    ...
    _isMultipleCurrentPhonesEnabled = false;
    ...
    get isMultipleCurrentPhonesEnabled() {
        return this._isMultipleCurrentPhonesEnabled;
    }
    ...
    set isMultipleCurrentPhonesEnabled(isMultipleCurrentPhonesEnabled) {
        this._isMultipleCurrentPhonesEnabled = isMultipleCurrentPhonesEnabled;
        this._lightningElement.componentAttributes.isMultipleCurrentPhonesEnabled = isMultipleCurrentPhonesEnabled;
    }
    ...
    setComponentViewAttributes(response) {
        let phoneRecords = response?.recordListWithStagedChanges;

        if (phoneRecords && phoneRecords.length > 0) {
            this.recordList = this.formatPhoneRecords(phoneRecords);
        }

        this.sectionConfiguration = JSON.parse(response?.configString)?.sectionConfigurationMap?.[this._lightningElement.sectionId];
    }

    formatPhoneRecords(phoneRecords) {
        let tempPhoneRecords = [];

        for (let eachPhone of phoneRecords) {
            let tempPhone = Object.assign({}, eachPhone);
            tempPhone.phoneNumber = this.formatPhoneNumber(tempPhone.phoneNumber);
            tempPhoneRecords.push(tempPhone);
        }

        return tempPhoneRecords;
    }
    ...
    formatPhoneNumber(phoneNumber) {
        ...
        return this.formatTenDigitPhoneNumber(cleanPhoneNumber);
    }
    ...
    formatTenDigitPhoneNumber(phoneNumber) {
        ...
        const formattedTenDigitPhoneNumber = `(${firstDigitGroup}) ${secondDigitGroup}-${thirdDigitGroup}`;
    ...
    setComponentEditAttributes(response) {
        ...
        this.isMultipleCurrentPhonesEnabled = configuration?.isMultipleCurrentPhonesEnabled;
        ...
        for (let eachRecord of response?.recordListWithStagedChanges) {
            recordListClone.push({
                                    ...
                                    phoneTypePicklistValues: this.getPhoneTypePicklistValues(eachRecord.phoneType)
                                });
        }
    ...
    get newPhone() {
        let phoneTypePicklistValues = this.getPhoneTypePicklistValues();
        return {
            Id: null,
            phoneNumber: "",
            phoneType: phoneTypePicklistValues[0].value,
            phoneIsPreferred: false,
            isDisplayOnPortal: true,
            phoneShowOnDirectory: false,
            status: "Active",
            timestampHtmlKey: Date.now(),
            phoneTypePicklistValues: phoneTypePicklistValues,
            isDelete: false
        };
    }

    handleAddPhone() {
        this.recordList.push(this.newPhone);
        this._lightningElement.componentAttributes.recordList = [...this.recordList];

        if (!this.isMultipleCurrentPhonesEnabled) {
            this.setPhoneTypePicklists();
        }
    }

    handleDeletePhone(recordIndex) {
    ...
    handleChange(value, fieldId, recordIndex) {
        this.recordList[recordIndex][fieldId] = value;
        if (fieldId === 'phoneIsPreferred' && value === true) {
            this.setPhonesToUnpreferred(recordIndex);
        }

        if (fieldId === 'phoneType' && !this.isMultipleCurrentPhonesEnabled) {
            this.setPhoneTypePicklists();
        }
    }

    setPhonesToUnpreferred(recordIndex) {
        for (let eachPhoneIndex = 0; eachPhoneIndex < this.recordList.length; eachPhoneIndex++) {
            if (eachPhoneIndex !== recordIndex) {
                this.recordList[eachPhoneIndex].phoneIsPreferred = false;
            }
        }
    ...
    setPhoneTypePicklists() {
        for (let eachRecord of this.recordList) {
            eachRecord.phoneTypePicklistValues = this.getPhoneTypePicklistValues(eachRecord.phoneType);
        }

        this.recordList = [...this.recordList];
    }

    getPhoneTypePicklistValues(phoneType) {
        if (this.isMultipleCurrentPhonesEnabled) {
            return this.sectionConfiguration.fieldConfigurationMap.phoneType.picklistValues;
        }

        let existingPicklistValues = new Set(this.recordList.map(eachRecord => !eachRecord.isDelete ? eachRecord.phoneType : ''));
        let phoneTypePicklistValues = this.sectionConfiguration.fieldConfigurationMap.phoneType.picklistValues.filter(eachValue => (eachValue.value == phoneType || !existingPicklistValues.has(eachValue.value)));
        return phoneTypePicklistValues;
    }
