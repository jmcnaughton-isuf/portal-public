...
export default class Portal_OnlineGivingEventCenter extends LightningElement {

    _dataMap = {};
    _receiptData = {};
    _subscription = null;
    _isDisableNavigationInterrupt = false;
    @wire(MessageContext) messageContext;


    connectedCallback() {
        this.intializeNavigationInterrupt()
        ...
        this._dataMap['lastPledgePaymentAmount'] = undefined;
    }
    ...
    handleMessage(message) {
        if (message.type == "amount") {
            ...
            publish(this.messageContext, onlineGivingChannel, {detail: this._dataMap, type: 'data-change'}); 
        } else if (message.type == 'lastPledgePaymentAmount') {
            this._dataMap['lastPledgePaymentAmount'] = message.detail;
            publish(this.messageContext, onlineGivingChannel, {detail: this._dataMap, type: 'data-change'}); 
        } else if (message.type == 'setHasAddressAutocomplete') {
            publish(this.messageContext, onlineGivingChannel, {detail: message.detail, type: 'hasAddressAutocomplete'}); 
        }
    }
