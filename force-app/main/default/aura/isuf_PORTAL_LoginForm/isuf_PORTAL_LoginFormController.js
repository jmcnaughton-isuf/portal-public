({
    initialize: function(component, event, helper) {
        ...
        component.set("v.isEnableRecaptcha", helper.getIsRecaptchaEnabled(component, event, helper))
        
    },
    handleValueInitParent: function(component, event, helper) {
        console.log('Controller function called');
        var facebookURL = component.get('v.facebookURL');
        var googleURL = component.get('v.googleURL');
        var facebookPlatform = 'Facebook';
        var googlePlatform = 'Google';

        var updateSocialLoginLinkFacebook = $A.get("e.c:updateSocialLoginLink");
        updateSocialLoginLinkFacebook.setParams({ 
            "url": facebookURL,
            "platform": facebookPlatform
        });
        updateSocialLoginLinkFacebook.fire();

        
        var updateSocialLoginLinkGoogle = $A.get("e.c:updateSocialLoginLink");
        updateSocialLoginLinkGoogle.setParams({ 
            "url": googleURL,
            "platform": googlePlatform
        });
        updateSocialLoginLinkGoogle.fire();
    ...
    handleRecaptchaResponse: function(component, event, helper) {
        helper.handleLoginWithRecaptchaV3(component, event, helper);
    },

    handleCloseAlert: function(component, event, helper){
        helper.handleCloseAlert(component, event, helper);
    },
